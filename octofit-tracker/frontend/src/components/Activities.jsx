...
function Activities() {
  ...
  useEffect(() => { const controller = new AbortController(); fetch(activitiesEndpoint, { signal: controller.signal }).then((response) => { if (!response.ok) throw new Error('Não foi possível carregar activities.'); return response.json() }).then((payload) => getItems(payload)).then(setActivities).then(() => setState('ready')).catch((requestError) => { if (requestError.name !== 'AbortError') { setError(requestError.message); setState('error') } }); return () => controller.abort() }, [])
